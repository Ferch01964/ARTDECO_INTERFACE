...
c 
c References:
c       Knut Stamnes, S-Chee Tsay, Warren Wiscombe, and Kolf Jayaweera, 
c       Numerically stable algorithm for discrete-ordinate-method radiative 
c       transfer in multiple scattering and emitting layered media
c        Applied Optics, Vol. 27, Issue 12, pp. 2502-2509 (1988)
c      
c ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      SUBROUTINE DISORT(NOCHECK, WARN, 
     &     MXCLY, MXULV, MXCMU, MXUMU, MXPHI, 
     &     MI, MI9M2, NNLYRI,
...
c     +-------------------------------------------------------------------+
      ...
      INTEGER   MI, MI9M2, NNLYRI

      INTEGER MXSQT
      PARAMETER (MXSQT = 1000)
c     ..
c     .. Scalar Arguments ..
...
c------------------------
c      Modified by Mathieu Compiegne for case NUMU (then MXUMU an MAXUMU = 1)
c      not to crash:
      IF (MAXUMU .ge.2) THEN
     ...
     &        NAZ = 0
      ELSE
         IF( FBEAM.EQ.0.0 .OR. ABS(1.-UMU0).LT.1.E-5 .OR. ONLYFL .OR.
     &        ( NUMU.EQ.1 .AND. ABS(1.-UMU(1)).LT.1.E-5 ) .OR.
     &        ( NUMU.EQ.1 .AND. ABS(1.+UMU(1)).LT.1.E-5 ) )
     &        NAZ = 0
      ENDIF
c------------------------
...
c     ** and emissivity properties
         IF( .NOT.LYRCUT )
     &        CALL SURFAC( NSTR, 
     &        ALBEDO, DELM0, CMU, FBEAM, LAMBER, MI, MAZIM,
            ...
            DO 170 J = 1, NPHI
               ...
               DO 160 LU = 1, NTAU
                  ...
                  DO 150 IU = 1, NUMU
                    ...
                    !write(*,*)   '2) UU, LU --->', UU( IU, LU, J ),LU
c                     if (UMU(IU) .lt. 0.0 ) then
c                        write(*,*)   'IU, LU, J, UU, UMU --->',
c     &            IU, LU, J, UU( IU, LU, J ), UMU(IU)
c                    end if
...
c                if(GL( K, LC ) .eq. 0.) then
c                write(*,*) 'GL, K, LC,OPRIM,PMOM', K, LC, OPRIM( LC ),
c      &          PMOM( K,LC ) 
c             end if
   20       CONTINUE
...
c      write(*,*) 'CMU in od_disort(line 2696) --->  ', CMU
c      write(*,*) 'CWT in od_disort(line 2696) --->  ', CWT
...
c                               ** in lower locations
  ...
  130    CONTINUE
      ...
      !write(*,*) 'UMU in od_disort(line 2696) --->  ', UMU
...
c     added by Mathieu Compiegne (Nov 2012)
      IF (NMUG .lt. 2*NSTR) THEN
         WRITE(*,*) ''
         WRITE(*,*) '(DISORT) In computing the BRDF'  
         WRITE(*,*) '(DISORT) NMUG may be too little'
         WRITE(*,*) '(DISORT) It should be > 2 * NSTR'
         WRITE(*,*) '(DISORT) NSTR=', NSTR
         WRITE(*,*) '(DISORT) NMUG=', NMUG
         WRITE(*,*) ''
      ENDIF
...
c                                  ** Beam source terms; Eq. SD(9)
   ...
   30       CONTINUE
            ...
            ZBEAM( IU ) = SUM
          !  if ( (SUM .eq. 0. ) .and. (IU .eq. 1)) THEN
          !    write(*,*) 'FACT',FACT
         !     DO  IQ = MAZIM, NSTR - 1
          !       IF ( YLMU( IQ, IU ) .eq. 0) then
          !            write(*,*) 'YLMU, IQ',IQ
          !         END IF
          !       write(*,*) 'YLM0', YLM0( IQ )
         !        write(*,*) 'PSI0',PSI0( IQ+1 ) 
         !         write(*,*) 'GL',GL( IQ )
         !     END DO
         !   end if
...
c                          ** integrate from bottom to LYU+1 in S1(9)
            PALINT = 0.0
            PLKINT = 0.0
            !write(*,*) 'in od_disort(USRINT) ,LYRCUT,LYU  ', LYRCUT, LYU
            DO 60 LC = LYRSTR, LYREND

               DTAU = DTAUCP( LC )
               EXP1 = EXP( ( UTAUPR(LU) - TAUCPR(LC-1) ) / UMU( IU ) )
               EXP2 = EXP( ( UTAUPR(LU) - TAUCPR(LC)   ) / UMU( IU ) )
c               if (IU .eq. NUMU) then
c                   write(*,*) 'in od_disort(USRINT) ,EXP1, EXP2, DTAU',
c     &                  EXP1, EXP2, DTAU
c               END IF
...
c                  if ( (IU .eq. NUMU) .and. ( PALINT .eq. 0) )then
c                   write(*,*) 'in od_disort(USRINT) ZBEAM,EXPN,DENOM,
c     &               DTAU,EXP0,EXP1,EXP2,EXPBEA ',
c     &                  ZBEAM( IU, LC ),EXPN,DENOM ,DTAU,EXP0,EXP1,EXP2
c               END IF
                 ! if ( ( EXPN .eq. 0. ) .and. ( IU .eq. NUMU ) ) then
                 !     write(*,*)  '1 )EXPN = 0, ZBEAM',ZBEAM( IU, LC )
                 !     write(*,*)  'press any key to continue'
                 !     read *
                 ! end if
                 ! if((ZBEAM(IU, LC).eq.0.).and.(IU.eq.NUMU)) then
                 !     write(*,*)  '1 ) ZBEAM = 0 EXPN',EXPN
                 !     write(*,*)  'press any key to continue'
                 !     read *
                 ! end if

                 ! write(*,*) 'PALINT 1 ', PALINT
     ...
     &    INPERR = WRTBAD( 'NMOM' )
      ...
      DO 30 LC = 1, NLYR
         ...
         DO 20 K = 0, NMOM
            ...
            write(*,*) 'PMOM , K,LC', PMOM( K,LC ), K,LC
               INPERR = WRTBAD( 'PMOM' )
            end if
